...
long long int sum_simd(int vals[NUM_ELEMS]) {
    ...
    for(unsigned int w = 0; w < OUTER_ITERATIONS; w++) {
        ...
        for (unsigned int i = 0; i < NUM_ELEMS / 4 * 4; i += 4) {
            __m128i tmp = _mm_loadu_si128((__m128i *)(vals + i));   // load 4 int from vals[]
            __m128i select = _mm_cmpgt_epi32(tmp, _127);   // select elements > 127
            tmp = _mm_and_si128(tmp, select);   // set unselected elements to 0
...
long long int sum_simd_unrolled(int vals[NUM_ELEMS]) {
    ...
    for(unsigned int w = 0; w < OUTER_ITERATIONS; w++) {
        ...
        for (unsigned int i = 0; i < NUM_ELEMS / 16 * 16; i += 16) {
            ...
            __m128i select = _mm_cmpgt_epi32(tmp, _127);   
            tmp = _mm_and_si128(tmp, select);   
            sum_vec = _mm_add_epi32(sum_vec, tmp);   

            tmp = _mm_loadu_si128((__m128i *)(vals + i + 4));  
            select = _mm_cmpgt_epi32(tmp, _127);   
            tmp = _mm_and_si128(tmp, select);   
            sum_vec = _mm_add_epi32(sum_vec, tmp);   
